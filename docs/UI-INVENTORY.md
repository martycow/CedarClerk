...
| Export button + Export modal | topbar `.export-trigger` button → `app-modal` (moved out of `<header>` 25.07.2026, see Bug 3 below) | button + modal | Channels (moved here from the topbar, B21), a checkbox per destination gating its settings (B5), blog publish/unpublish, Telegram schedule, privacy + invite list, registration-form editor and submissions (B3), `.cedar`/static-HTML export, per-draft asset list with total size. **One Publish button** at the bottom fires every ticked destination. **Reworked 26.07.2026 (N4/N5/N13, ADR-045)**: 1180px wide with a `repeat(auto-fit, minmax(330px, 1fr))` grid over the sections; a Telegram target is chosen by clicking a connected channel (`.channel-pick`) with no text field; an unticked destination folds to its header (`.dest-off`) keeping only the LIVE chip; connect-by-@username hides behind a `.manual-channel-toggle` disclosure | Needed & present — `exporting()`/`blogBusy()`/`scheduling()`/`publishingAll()` each show a spin icon + `.inline-progress` indeterminate bar | By far the largest modal in the app and still growing; was mis-positioned near the top of the screen until the Bug 3 fix (25.07.2026). Folding the blog destination also hides the privacy/invite/registration-form controls nested in it — one tick away, see ADR-045 |
...
| New-draft dialog | `app-modal`, `closeNewDraftDialog()` | modal | Title, languages, tags (usage-counted pill cloud + free-text input, N2), template, target folder, and a "private" checkbox for a new draft. Folder/private are applied as follow-up calls right after creation (the create endpoint doesn't take them) and, unlike languages/tags/template, are **not** remembered in `newDraftDefaultsJson` — they're per-draft intent, not a preference | Needed & present — `draftsBusy()` drives the "Creating…" button state | Folder shown as a pill row, not the editor's folder popover: a nested `app-popover` inside `app-modal` fights the modal's own fixed positioning |
...
| Forms tab — submissions list | `.registration-row` | panel | Who submitted a private post's registration form, when, and what they answered | Needed & present — `registrationsLoading()` | Answers now resolve to question labels, falling back to the raw key for questions deleted after the fact |
