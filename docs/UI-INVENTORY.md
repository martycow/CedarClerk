...
| `app-debug-console` | `debug-console.component.ts`, mounted once in the root app shell | floating tab + panel | Dev tool: inspect in-flight/failed API requests without SSH-ing into the Pi, available on every page | N/A (dev tool) | Fixed `bottom:0` overlay, always on top (`z-index:200`); closed-tab now sits 27px above the true bottom edge (`margin-bottom`) so it clears the editor's status bar instead of covering the word-count/sync indicator (fixed 25.07.2026) |
...
| Element | Location | Type | Purpose | Loading state | Notes |
|---|---|---|---|---|---|
| Back-to-editor link | `:3` | button | Nav to `/editor` | N/A | |
