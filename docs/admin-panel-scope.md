...
- **Step 3 — done 27.07.2026.** New `InviteCode` entity + `ApplicationUser.InviteCodeId` (migration `AddInviteCodes`). Registration checks real codes first and falls back to `Cedar:InviteCode`, which stays exactly as decided. Admin can create codes (label, optional expiry, optional use cap), **deactivate but never delete** them — accounts point at the row, and deleting it would erase their attribution — and **set attribution by hand** on the accounts that predate tracking, logged as "set by hand" so an admin's assertion about history is never confused with something the system observed.
  - The "is this code still usable" predicate lives in `CedarClerk.Core/InviteCodeRules.cs` with unit tests, shared by registration and the panel. It was briefly written out twice; the copy that drifts is the one guarding registration, so it became one function covering the edges that matter (a cap of 5 admits exactly five accounts; expiry closes the code at the instant itself).
  - A limited code's use is counted only **after** the account is actually created, so a failed registration can't burn one.
- **Steps 4 and 5 — done 27.07.2026, admin panel complete.** Step 4: a read-only cross-owner post list (owner, state, views/comments, links out to the live blog and Telegram post) — nothing there writes, per the decision. Step 5: payments from the existing `Payment` table with a completed-only revenue total (a failed or pending row is not money), and per-user storage and AI-call reporting. All of it reads data that already existed; no new collection was added. The panel gained a tab strip at this point, matching the Posts Manager and Settings so the app's secondary pages behave alike.

**Live-verified by Marty 27.07.2026** (against the Step-1/2 build): `/api/admin/users` returns 404 for a signed-in non-admin, `/admin` redirects, self-targeting actions are refused. That closes the gate check the scoping flagged as untestable here.
